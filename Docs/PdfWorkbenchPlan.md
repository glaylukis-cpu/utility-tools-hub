...
v0.4.1 focuses on PDF Workbench QA polish, responsive layout checks, safety wording, and summary UI readability. No new PDF processing, preview, reorder, OCR, redaction, or direct text editing is added.
...
**v0.4.0 Step 4B - Merge multi-file summary**

Merge PDFsの各入力へ`pdf_inspect`を実行し、選択順、サイズ、ページ数、PDF version、合計ページ数、保護状態の警告を表示します。プレビュー、サムネイル、並び替え、新しいPDF処理は引き続き予定機能です。
...
Additiveなtext watermarkを新しいPDFへ出力するcore / bridgeを追加します。UI接続とpreviewはまだ追加しません。
...
**v0.7.1 - Page numbers real PDF QA:** Compact PDF WorkbenchでPage numbersを検証し、layout overflow fixesを維持します。新しいPDF処理カテゴリは追加しません。
...
JPEG-only Image watermark coreをshared execution bridgeへ接続します。UIとpreviewはまだ追加しません。

**v0.8.0 Step 4 - JPEG Image watermark UI connection**

JPEG-only Image watermarkをcompact PDF Workbench UIへ接続します。既存のlayout overflow fixesを維持し、previewは追加しません。

**v0.8.0 Step 5 - JPEG Image watermark QA**

Compact PDF WorkbenchでJPEG-only Image watermarkの実PDF出力とvalidationを確認し、既存のlayout overflow fixesを維持します。新しいPDF処理カテゴリは追加しません。
